...
import type { MatchProfile, ExchangeRequest } from "@/types/index";
...
type FilterFormat = "all" | "online" | "offline";
...
function FilterPanel({
  ...
  level: FilterLevel;
  format: FilterFormat;
  onLevelChange: (v: FilterLevel) => void;
  ...
  const formats: { value: FilterFormat; label: string }[] = [
  ...
  return (
    <div className="rounded-xl border border-white/5 bg-zinc-900/70 p-4 mb-4 flex flex-wrap gap-6">
      <div>
        <p className="text-xs text-zinc-500 mb-2 font-bold uppercase tracking-widest">
          Уровень
        </p>
        <div className="flex gap-2 flex-wrap">
          {levels.map(({ value, label }) => (
            <button
              ...
              className={clsx(
                "rounded-lg border text-xs px-3 py-1.5 transition-colors",
                level === value
                  ? "border-blue-500/40 bg-blue-500/10 text-blue-400"
                  : "border-white/5 bg-zinc-800 text-zinc-400 hover:border-white/10 hover:text-zinc-200",
              )}
            >
              {label}
            </button>
...
export default function FindPage() {
  ...
  // true  = exists
  const [hasMatchProfile, setHasMatchProfile] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await api.getCandidatesBySkill(query.trim());
    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
      ...
      <div className="flex gap-3 mb-3">
        ...
        <button
          ...
          className={clsx(
            "flex items-center gap-2 rounded-xl border px-4 py-3 text-sm font-medium transition-colors shrink-0",
            showFilters
              ? "border-blue-500/40 bg-blue-500/10 text-blue-400"
              : "border-white/5 bg-zinc-800/70 text-zinc-400 hover:text-zinc-200 hover:border-white/10",
          )}
        >
